...
class EarlyStopping(object):
    ...
    def step(self, metrics: list, model: jt.Module):
        """
        ...
        if jt.all(jt.array(metrics_compare_results)):
            for metric_tuple in metrics:
    ...
    def save_checkpoint(self, model: jt.Module):
        """
        ...
        jt.save(model.state_dict(), self.save_model_path)
        if self.model_name in ['JODIE', 'DyRep', 'TGN']:
            # torch.save(model[0].memory_bank.node_raw_messages, self.save_model_nonparametric_data_path)
            jt.save(model[0].memory_bank.node_raw_messages, self.save_model_nonparametric_data_path)

    def load_checkpoint(self, model: jt.Module, map_location: str = None):
        """
        ...
        model.load_state_dict(jt.load(self.save_model_path))
        if self.model_name in ['JODIE', 'DyRep', 'TGN']:
            # model[0].memory_bank.node_raw_messages = torch.load(self.save_model_nonparametric_data_path, map_location=map_location)
            model[0].memory_bank.node_raw_messages = jt.load(self.save_model_nonparametric_data_path)
