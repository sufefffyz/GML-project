...
def evaluate_model_link_prediction(model_name: str, model: jt.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluate_data: Data, loss_func: jt.Module,
    ...
    with jt.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            predicts = jt.concat([positive_probabilities, negative_probabilities], dim=0)
            labels = jt.concat([jt.ones_like(positive_probabilities), jt.zeros_like(negative_probabilities)], dim=0)

            loss = loss_func(output=predicts, target=labels)
...
def evaluate_model_node_classification(model_name: str, model: jt.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                       evaluate_data: Data, loss_func: jt.Module, num_neighbors: int = 20, time_gap: int = 2000):
    """
    ...
    with jt.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            labels = jt.array(batch_labels).float32()
        ...
        evaluate_y_trues = jt.concat(evaluate_y_trues, dim=0)
        evaluate_y_predicts = jt.concat(evaluate_y_predicts, dim=0)
...
def evaluate_edge_bank_link_prediction(args: argparse.Namespace, train_data: Data, val_data: Data, test_idx_data_loader: DataLoader,
    ...
    for run in range(args.num_runs):
        ...
        for batch_idx, test_data_indices in enumerate(test_idx_data_loader_tqdm):
            ...
            predicts = jt.array(np.concatenate([positive_probabilities, negative_probabilities])).float32()
            labels = jt.concat([jt.ones(len(positive_probabilities)), jt.zeros(len(negative_probabilities))], dim=0)
