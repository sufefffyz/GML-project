...
#         nodes_edge_raw_features = self._edge_raw_features[jt.array(neighbor_edge_ids)]
...
#         nodes_neighbor_time_features = self.time_encoder(timestamps=jt.array(node_interact_times[:, np.newaxis] - neighbor_times).float())
#         # ndarray, set the time features to all zeros for the padded timestamp
#         # nodes_neighbor_time_features[torch.from_numpy(neighbor_node_ids == 0)] = 0.0
#         nodes_neighbor_time_features[jt.array(neighbor_node_ids == 0)] = 0.0
...
#         combined_features = jt.concat([nodes_edge_raw_features, nodes_neighbor_time_features], dim=-1)
...
#         nodes_time_gap_neighbor_node_raw_features = self._node_raw_features[jt.array(time_gap_neighbor_node_ids)]
...
#         valid_time_gap_neighbor_node_ids_mask = jt.array((time_gap_neighbor_node_ids > 0).astype(np.float32))
...
#         output_node_features = nodes_time_gap_neighbor_node_agg_features + self._node_raw_features[jt.array(node_ids)]

#         # Tensor, shape (batch_size, node_feat_dim)
#         # node_embeddings = self.output_layer(torch.cat([combined_features, output_node_features], dim=1))
#         node_embeddings = self.output_layer(jt.concat([combined_features, output_node_features], dim=1))
...
# class FeedForwardNet(jt.Module):
...
class GraphMixer(jt.Module):
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
        # nodes_edge_raw_features = self.edge_raw_features[torch.from_numpy(neighbor_edge_ids)]
        nodes_edge_raw_features = self.edge_raw_features[jt.array(neighbor_edge_ids)]
        # Tensor, shape (batch_size, num_neighbors, time_feat_dim)
        # nodes_neighbor_time_features = self.time_encoder(timestamps=torch.from_numpy(node_interact_times[:, np.newaxis] - neighbor_times).float().to(self.device))
        nodes_neighbor_time_features = self.time_encoder(timestamps=jt.array(node_interact_times[:, np.newaxis] - neighbor_times).float().to(self.device))
        ...
        # Tensor, shape (batch_size, time_gap, node_feat_dim)
        # nodes_time_gap_neighbor_node_raw_features = self.node_raw_features[torch.from_numpy(time_gap_neighbor_node_ids)]
        nodes_time_gap_neighbor_node_raw_features = self.node_raw_features[jt.array(time_gap_neighbor_node_ids)]
        ...
        # Tensor, shape (batch_size, node_feat_dim), add features of nodes in node_ids
        # output_node_features = nodes_time_gap_neighbor_node_agg_features + self.node_raw_features[torch.from_numpy(node_ids)]
        output_node_features = nodes_time_gap_neighbor_node_agg_features + self.node_raw_features[jt.array(node_ids)]
...
class FeedForwardNet(jt.Module):
    ...
    def __init__(self, input_dim: int, dim_expansion_factor: float, dropout: float = 0.0):
        ...
        self.dim_expansion_factor = dim_expansion_factor
        self.dropout = dropout
        print(input_dim, dim_expansion_factor, dropout)
        self.ffn = nn.Sequential(nn.Linear(in_features=input_dim, out_features=int(dim_expansion_factor * input_dim)),
                                 nn.GELU(),
                                #  nn.gelu,
                                 nn.Dropout(dropout),
...
# test = FeedForwardNet(input_dim=20, dim_expansion_factor=0.5, dropout=0.1)
# print(test.forward(jt.randn(10, 20)))
