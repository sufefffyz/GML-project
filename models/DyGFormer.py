...
class DyGFormer(jt.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        src_patches_nodes_neighbor_node_raw_features = self.projection_layer_node(src_patches_nodes_neighbor_node_raw_features)
        src_patches_nodes_edge_raw_features = self.projection_layer_edge(src_patches_nodes_edge_raw_features)
        src_patches_nodes_neighbor_time_features = self.projection_layer_time(src_patches_nodes_neighbor_time_features)
        src_patches_nodes_neighbor_co_occurrence_features = self.projection_layer_neighbor_co_occurrence(src_patches_nodes_neighbor_co_occurrence_features)

        # Tensor, shape (batch_size, dst_num_patches, channel_embedding_dim)
        dst_patches_nodes_neighbor_node_raw_features = self.projection_layer_node(dst_patches_nodes_neighbor_node_raw_features)
        dst_patches_nodes_edge_raw_features = self.projection_layer_edge(dst_patches_nodes_edge_raw_features)
        dst_patches_nodes_neighbor_time_features = self.projection_layer_time(dst_patches_nodes_neighbor_time_features)
        dst_patches_nodes_neighbor_co_occurrence_features = self.projection_layer_neighbor_co_occurrence(dst_patches_nodes_neighbor_co_occurrence_features)
        ...
        patches_nodes_neighbor_node_raw_features = jt.concat([src_patches_nodes_neighbor_node_raw_features, dst_patches_nodes_neighbor_node_raw_features], dim=1)
        patches_nodes_edge_raw_features = jt.concat([src_patches_nodes_edge_raw_features, dst_patches_nodes_edge_raw_features], dim=1)
        patches_nodes_neighbor_time_features = jt.concat([src_patches_nodes_neighbor_time_features, dst_patches_nodes_neighbor_time_features], dim=1)
        patches_nodes_neighbor_co_occurrence_features = jt.concat([src_patches_nodes_neighbor_co_occurrence_features, dst_patches_nodes_neighbor_co_occurrence_features], dim=1)
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        padded_nodes_neighbor_time_features[jt.array(padded_nodes_neighbor_ids == 0)] = 0.0
    ...
    def get_patches(self, padded_nodes_neighbor_node_raw_features: jt.Var, padded_nodes_edge_raw_features: jt.Var,
        ...
        patches_nodes_neighbor_node_raw_features = jt.stack(patches_nodes_neighbor_node_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.node_feat_dim)
        ...
        patches_nodes_edge_raw_features = jt.stack(patches_nodes_edge_raw_features, dim=1).reshape(batch_size, num_patches, patch_size * self.edge_feat_dim)
        ...
        patches_nodes_neighbor_time_features = jt.stack(patches_nodes_neighbor_time_features, dim=1).reshape(batch_size, num_patches, patch_size * self.time_feat_dim)
        ...
        patches_nodes_neighbor_co_occurrence_features = jt.stack(patches_nodes_neighbor_co_occurrence_features, dim=1).reshape(batch_size, num_patches, patch_size * self.neighbor_co_occurrence_feat_dim)
...
class NeighborCooccurrenceEncoder(jt.Module):
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
        ...
        for src_padded_node_neighbor_ids, dst_padded_node_neighbor_ids in zip(src_padded_nodes_neighbor_ids, dst_padded_nodes_neighbor_ids):
            ...
            src_padded_node_neighbor_counts_in_src = jt.array(src_counts[src_inverse_indices]).float()
            ...
            dst_padded_node_neighbor_counts_in_dst = jt.array(dst_counts[dst_inverse_indices]).float()
            ...
            src_padded_node_neighbor_counts_in_dst = jt.array([dst_mapping_dict.get(neighbor_id, 0.0) for neighbor_id in src_padded_node_neighbor_ids.copy()]).float()
            ...
            src_padded_nodes_appearances.append(jt.stack([src_padded_node_neighbor_counts_in_src, src_padded_node_neighbor_counts_in_dst], dim=1))
            ...
            dst_padded_node_neighbor_counts_in_src = jt.array([src_mapping_dict.get(neighbor_id, 0.0) for neighbor_id in dst_padded_node_neighbor_ids.copy()]).float()
            ...
            dst_padded_nodes_appearances.append(jt.stack([dst_padded_node_neighbor_counts_in_src, dst_padded_node_neighbor_counts_in_dst], dim=1))

        # Tensor, shape (batch_size, src_max_seq_length, 2)
        # src_padded_nodes_appearances = torch.stack(src_padded_nodes_appearances, dim=0)
        src_padded_nodes_appearances = jt.stack(src_padded_nodes_appearances, dim=0)
        # Tensor, shape (batch_size, dst_max_seq_length, 2)
        # dst_padded_nodes_appearances = torch.stack(dst_padded_nodes_appearances, dim=0)
        dst_padded_nodes_appearances = jt.stack(dst_padded_nodes_appearances, dim=0)
        ...
        src_padded_nodes_appearances[jt.array(src_padded_nodes_neighbor_ids == 0)] = 0.0
        # Tensor, shape (batch_size, dst_max_seq_length, 2)
        # dst_padded_nodes_appearances[torch.from_numpy(dst_padded_nodes_neighbor_ids == 0)] = 0.0
        dst_padded_nodes_appearances[jt.array(dst_padded_nodes_neighbor_ids == 0)] = 0.0
...
class TransformerEncoder(jt.Module):
    ...
    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1):
        ...
        self.multi_head_attention = attention.MultiheadAttention(embed_dim=attention_dim, num_heads=num_heads)
    ...
    def execute(self, inputs: jt.Var):
        ...
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
        hidden_states = self.linear_layers[1](self.dropout(nn.gelu(self.linear_layers[0](self.norm_layers[1](outputs)))))
        # Tensor, shape (batch_size, num_patches, self.attention_dim)
