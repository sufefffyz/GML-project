...
class TimeEncoder(jt.Module):
    ...
    def __init__(self, time_dim: int, parameter_requires_grad: bool = True):
        ...
        self.w.weight = nn.Parameter((jt.array(1 / 10 ** np.linspace(0, 9, time_dim, dtype=np.float32))).reshape(time_dim, -1))
...
class MergeLayer(jt.Module):
    ...
    def execute(self, input_1: jt.Var, input_2: jt.Var):
        ...
        x = jt.concat([input_1, input_2], dim=1)
...
class MultiHeadAttention(jt.Module):
    ...
    def execute(self, node_features: jt.Var, node_time_features: jt.Var, neighbor_node_features: jt.Var,
                neighbor_node_time_features: jt.Var, neighbor_node_edge_features: jt.Var, neighbor_masks: np.ndarray):
        """
        ...
        key = value = jt.concat([neighbor_node_features, neighbor_node_edge_features, neighbor_node_time_features], dim=2)
        ...
        attention_mask = jt.array(neighbor_masks).unsqueeze(dim=1)
        ...
        attention_mask = jt.stack([attention_mask for _ in range(self.num_heads)], dim=1)
...
class TransformerEncoder(jt.Module):
    ...
    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1):
        ...
        self.multi_head_attention = jt.attention.MultiheadAttention(embed_dim=attention_dim, num_heads=num_heads)
    ...
    def execute(self, inputs_query: jt.Var, inputs_key: jt.Var = None, inputs_value: jt.Var = None,
                neighbor_masks: jt.Var = None):
        """
        ...
        if neighbor_masks is not None:
            ...
            neighbor_masks = jt.array(neighbor_masks) == 0
        ...
        # Tensor, shape (batch_size, target_seq_length, self.attention_dim)
        hidden_states = self.linear_layers[1](self.dropout(nn.relu(self.linear_layers[0](outputs))))
        # Tensor, shape (batch_size, target_seq_length, self.attention_dim)
