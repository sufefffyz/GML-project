...
class MemoryModel(jt.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if self.model_name == 'JODIE':
            ...
            src_node_time_intervals = jt.array(node_interact_times).float32() - updated_node_last_updated_times[jt.array(src_node_ids)]
            ...
            dst_node_time_intervals = jt.array(node_interact_times).float32() - updated_node_last_updated_times[jt.array(dst_node_ids)]
            ...
            node_time_intervals = jt.concat([src_node_time_intervals, dst_node_time_intervals], dim=0)
        ...
        if self.model_name == 'DyRep':
            ...
            src_node_embeddings = updated_node_memories[jt.array(src_node_ids)]
            # dst_node_embeddings = updated_node_memories[torch.from_numpy(dst_node_ids)]
            dst_node_embeddings = updated_node_memories[jt.array(dst_node_ids)]
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, dst_node_embeddings: jt.Var,
        ...
        src_node_delta_times = jt.array(node_interact_times).float32() - \
                               self.memory_bank.node_last_updated_times[jt.array(src_node_ids)]
        ...
        new_src_node_raw_messages = jt.concat([src_node_memories, dst_node_memories, src_node_delta_time_features, edge_features], dim=1)
...
class MessageAggregator(jt.Module):
    ...
    def aggregate_messages(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        unique_node_messages = jt.stack(unique_node_messages, dim=0) if len(unique_node_messages) > 0 else jt.array([])
...
class MemoryBank(jt.Module):
    ...
    def get_memories(self, node_ids: np.ndarray):
        ...
        return self.node_memories[jt.array(node_ids)]

    def set_memories(self, node_ids: np.ndarray, updated_node_memories: jt.Var):
        """
        ...
        self.node_memories[jt.array(node_ids)] = updated_node_memories
    ...
    def get_node_last_updated_times(self, unique_node_ids: np.ndarray):
        ...
        return self.node_last_updated_times[jt.array(unique_node_ids)]
...
class MemoryUpdater(jt.Module):
    ...
    def update_memories(self, unique_node_ids: np.ndarray, unique_node_messages: jt.Var,
        ...
        self.memory_bank.node_last_updated_times[jt.array(unique_node_ids)] = jt.array(unique_node_timestamps).float32()

    def get_updated_memories(self, unique_node_ids: np.ndarray, unique_node_messages: jt.Var,
                             unique_node_timestamps: np.ndarray):
        """
...
class TimeProjectionEmbedding(jt.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: jt.Var, node_ids: np.ndarray, node_time_intervals: jt.Var):
        """
        ...
        """
        # Tensor, shape (batch_size, memory_dim)
        source_embeddings = self.dropout(node_memories[jt.array(node_ids)] * (1 + self.linear_layer(node_time_intervals.unsqueeze(dim=1))))

        return source_embeddings


class GraphAttentionEmbedding(jt.Module):

    def __init__(self, node_raw_features: jt.Var, edge_raw_features: jt.Var, neighbor_sampler: NeighborSampler,
    ...
    def compute_node_temporal_embeddings(self, node_memories: jt.Var, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            neighbor_time_features = self.time_encoder(timestamps=jt.array(neighbor_delta_times).float32())
